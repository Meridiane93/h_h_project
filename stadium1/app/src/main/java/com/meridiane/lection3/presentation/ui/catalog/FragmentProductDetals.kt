    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch {
            viewModel.productsState.collectLatest { product ->
                ...
                if (product.title != null) {
                    ...
                    binding.tittleAppBar.text = if (product.title.length > 28) product.title.substring(0, 28)
                                                else product.title
        ...
        binding.btAddOrder.setOnClickListener{
            if(viewModel.sizeProduct == ""){
                Toast.makeText(requireContext(), getString(R.string.not_size),Toast.LENGTH_LONG).show()
            }
            else {
