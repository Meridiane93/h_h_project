...
class MockRepository(
    ...
    private val coordinateStorage: CoordinateStorage
) :
    ...
    InterfaceClearCoordinate,
    InterfaceGetCoordinate,
    InterfaceSaveCoordinate {

    // добавление токена в запрос
    private val client = OkHttpClient.Builder()
        .addInterceptor { chain ->
            val original = chain.request()
            val token = getTokenBd().substringAfter("(")
                .replace(")", "", true)

            val requestBuilder = original.newBuilder()
                .header("Authorization", "Bearer $token") // брал из константы
    ...
    override fun getTokenBd(): String = tokenStorage.getToken() // получить токен из преференс
    override fun saveToken(token: String) = tokenStorage.saveToken(token) // сохранение токена
    override fun clearTokenBd() = tokenStorage.clearToken() // удаление токена

    override fun clearCoordinateBd() = coordinateStorage.clearCoordinate() // удалить координаты из преференс
    override fun getCoordinateBd(): String = coordinateStorage.getCoordinate() // получить координаты из преференс
    override fun saveCoordinate(coordinate: String) = coordinateStorage.saveCoordinate(coordinate) // сохранить координаты+
